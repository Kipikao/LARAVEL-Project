<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit caiet service') }}
        </h2>
    </x-slot>

    <form method="post" action="{{ route('notebooks.update',['caiet' => $caiet]) }}">
        @csrf
        @method('put')
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <label class="block font-medium text-sm text-gray-700 dark:text-gray-300">
                            ...
                            shadow-sm mt-1 block w-full" value="{{ $caiet->carNumber }}">
                        </label>
                        <label class="block font-medium text-sm text-gray-700 dark:text-gray-300">
                            ...
                            focus:border-indigo-500 dark:focus:border-indigo-600 
                            focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md 
                            shadow-sm mt-1 block w-full" value="{{ $caiet->kmCar }}">
                        </label>
                        <label class="block font-medium text-sm text-gray-700 dark:text-gray-300">
                            <textarea name="store" cols="30" rows="10" class="border-gray-300 dark:border-gray-700 
                            dark:bg-gray-900 dark:text-gray-300 
                            focus:border-indigo-500 dark:focus:border-indigo-600 
                            focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md 
                            shadow-sm mt-1 block w-full">{{ $caiet->store }}
                            </textarea>
                        </label>
                        <div class="py-12">
                            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
